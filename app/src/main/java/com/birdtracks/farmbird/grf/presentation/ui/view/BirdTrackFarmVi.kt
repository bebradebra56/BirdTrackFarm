...
class BirdTrackFarmVi(
    private val birdTrackFarmContext: Context,
    private val birdTrackFarmCallback: BirdTrackFarmCallBack,
    private val birdTrackFarmWindow: Window
) : WebView(birdTrackFarmContext) {
    private var birdTrackFarmFileChooserHandler: ((ValueCallback<Array<Uri>>?) -> Unit)? = null
    fun birdTrackFarmSetFileChooserHandler(handler: (ValueCallback<Array<Uri>>?) -> Unit) {
        this.birdTrackFarmFileChooserHandler = handler
    }
    init {
        ...
        webSettings.apply {
            ...
            userAgentString = WebSettings.getDefaultUserAgent(birdTrackFarmContext).replace("; wv)", "").replace("Version/4.0 ", "")
        ...
        super.setWebViewClient(object : WebViewClient() {
            override fun shouldOverrideUrlLoading(
                ...
                return if (request?.isRedirect == true) {
                    ...
                    try {
                        birdTrackFarmContext.startActivity(intent)
                    } catch (e: Exception) {
                        Toast.makeText(birdTrackFarmContext, "This application not found", Toast.LENGTH_SHORT).show()
                    }
                    true
                }
            }
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (url?.contains("https://test-web.syndi-test.net") != true) {
                    BirdTrackFarmApplication.birdTrackFarmInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN
                    Log.d(BirdTrackFarmApplication.BIRD_TRACK_FARM_MAIN_TAG, "onPageFinished : ${BirdTrackFarmApplication.birdTrackFarmInputMode}")
                    birdTrackFarmWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);
                    birdTrackFarmEnableStableInputScroll()
                } else {
                    BirdTrackFarmApplication.birdTrackFarmInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE
                    Log.d(BirdTrackFarmApplication.BIRD_TRACK_FARM_MAIN_TAG, "onPageFinished : ${BirdTrackFarmApplication.birdTrackFarmInputMode}")
                    birdTrackFarmWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
                }
            }
        ...
        super.setWebChromeClient(object : WebChromeClient() {
            ...
            override fun onShowFileChooser(
                ...
                birdTrackFarmFileChooserHandler?.invoke(filePathCallback)
                return true
            }
            override fun onCreateWindow(
                ...
                birdTrackFarmHandleCreateWebWindowRequest(resultMsg)
                return true
            }
        })
    }
    ...
    fun birdTrackFarmFLoad(link: String) {
        super.loadUrl(link)
    }

    private fun birdTrackFarmEnableStableInputScroll() {
        evaluateJavascript(
            """
        (function() {
    ...
    private fun birdTrackFarmHandleCreateWebWindowRequest(resultMsg: Message?) {
        if (resultMsg == null) return
        if (resultMsg.obj != null && resultMsg.obj is WebView.WebViewTransport) {
            val transport = resultMsg.obj as WebView.WebViewTransport
            val windowWebView = BirdTrackFarmVi(birdTrackFarmContext, birdTrackFarmCallback, birdTrackFarmWindow)
            transport.webView = windowWebView
            resultMsg.sendToTarget()
            birdTrackFarmCallback.birdTrackFarmHandleCreateWebWindowRequest(windowWebView)
        }
    }
